...
### Summarize it, and give them the link

The change result contains the diff in `structuredContent` and text. Summarize
what would be created, changed or removed, call out assumptions and questions,
and give the person `approval_url`. "Approve it in Basepath" without the link
leaves them nothing to click, and a proposal expires in thirty minutes.
