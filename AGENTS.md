...
- The home view should keep the goal map as the primary content: show the template chooser only until the workspace has saved plan content, omit the always-visible explanatory root node, keep goal cards compact, group less-frequent sidebar destinations under a collapsed 「その他」 menu, show the active tenant beside the compact workspace switcher, and reveal a compact plus icon at a node boundary on hover for adding a child (user feedback, 2026-09-21).
