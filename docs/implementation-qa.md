...
- One real child-process MCP stdio test: initialization, 40 tools including the retrieval, breakdown, conversation-plan-draft and copilot contracts, resources, prompts, proposal-only behavior, rejected unapproved apply, the person's approval being what writes, and an apply arriving afterwards reporting `already_applied` without writing again.
