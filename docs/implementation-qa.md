...
- One real child-process MCP stdio test: initialization, 34 tools including the retrieval, breakdown and copilot contracts, resources, prompts, proposal-only behavior, rejected unapproved apply, the person's approval being what writes, and an apply arriving afterwards reporting `already_applied` without writing again.
