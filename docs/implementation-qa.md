...
Not done here: the user pool has no PathBase App Client yet. Provisioning one needs Cognito administration on the Tachyon platform account — the `OAuth2Client` manifest cannot create it, because `useTachyonUserPool` with `clientType: public` currently registers a Tachyon-internal client only, and the backlog task PLT-2582 that would provision a public Cognito client is unimplemented. Until that client exists, production keeps the legacy flow and Field stays at 401, so this change is verified against local mocks rather than the live Field environment.
