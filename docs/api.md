...
- 本番はTachyon認証済みの`pathbase_session` Cookieを使います。`GET /auth/login`でOIDCへ遷移、`GET /auth/callback`でセッション確立、`POST /auth/logout`でPathBaseからログアウト。`GET /auth/status`と`GET /health`は公開の設定状況・ヘルス情報です。
