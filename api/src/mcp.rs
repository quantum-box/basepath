...
fn tools() -> Vec<Tool> {
    let defs = [
        ...
        read("pathbase_get_graph", "Get the goal graph for one workspace and its `plan_version`, the opaque baseline required by pathbase_preview_changes. Returns at most `limit` nodes (default and maximum 200) with the relations between them, plus `truncated` and the `limit` that was applied. When `truncated` is true the graph is a slice, not the plan: narrow the request or read the missing subtree with pathbase_get_item. For a change proposal, re-read the full relevant graph immediately before preview and pass that response's plan_version as expected_base_version."),
