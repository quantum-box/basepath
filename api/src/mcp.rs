...
/// The single MCP Apps surface for plan data.
///
/// The iframe is intentionally one reusable document. The workspace boundary
...
impl Mcp {
    ...
    fn authorize(
        ...
        // Reconciling another conversation with an existing plan needs both
        // proposal permission and permission to read that exact workspace.
        // A proposal-only connection must not infer matches from data it could
        // not read.
        if matches!(tool, "pathbase_preview_changes" | "pathbase_propose_plan")
            && args["conversation_id"].as_str().is_some()
        {
            identity.require(mcp_auth::SCOPE_READ)?;
        }
    ...
    pub async fn call(
        ...
        let actor = self.authorize(name, &args, extensions)?;
        ...
        let (method, path, body) = match name {
            "pathbase_get_context" => {
                ...
                let workspaces = self
                    ...
                    .await?;
                // Where a person approves a change. The host links here rather
                // than trying to collect approval itself: a click inside an
                // AI host reaches this server as an ordinary tool call, which
                // the server cannot distinguish from the model's, so it is not
        ...
        if is_change {
            ...
            value["where_to_approve"] = json!(match value["status"].as_str() {
                ...
                _ if value["auto_apply_eligible"] == json!(true) =>
                    "This falls inside a range the person set in Basepath in advance. Show them the diff, say it is inside a range they set — not that you have permission — and, if they explicitly ask to approve or reflect it in this conversation, call pathbase_apply_changes. If that is refused, nothing was written: show them this URL.",
                _ =>
...
fn argument_contract(name: &str) -> Option<(&'static [&'static str], &'static [&'static str])> {
    Some(match name {
        ...
        // A caller can request a bounded week explicitly; the response is
        // returned as structured data for the model to summarize.
        "pathbase_get_week" => (
...
fn tools() -> Vec<Tool> {
    let defs = [
        ...
        read("pathbase_get_graph", "Get the goal graph for one workspace and its `plan_version`. Returns at most `limit` nodes (default and maximum 200) with the relations between them, plus `truncated` and the `limit` that was applied. When `truncated` is true the graph is a slice, not the plan: narrow the request or read the missing subtree with pathbase_get_item. For a linked conversation integration, re-read the full relevant graph immediately before preview and pass that response's plan_version as the required expected_base_version. An ordinary proposal without conversation_id may omit the baseline."),
        ...
        read("pathbase_list_changes", "List saved change sets and their current status so the model can explain what is awaiting approval."),
        ...
        write("pathbase_preview_changes", "Validate and save a pending change set. Never applies the plan; a person reviews it in PathBase. For a linked conversation integration, read the graph first and pass its `plan_version` as the required `expected_base_version`; a stale baseline is rejected, so reread and rebuild the diff. Also pass the linked `conversation_id`; this requires both pathbase.read and pathbase.propose and is checked against the authorized workspace. Without `conversation_id`, expected_base_version is optional; do not use read tools unless pathbase.read is granted. Use existing item IDs for updates and moves; never identify an item by title alone. Every integration operation needs `match_rationale`: why it refers to the matched item, or why a new item is genuinely new. Do not delete items or remove an unmentioned branch. Relation removals need an explicit rationale. Preserve IDs, descendants, execution records and review history. If there is no change, send an empty operations array with the same conversation and baseline; the response is `no_change` and creates no pending changeset. Reuse the same idempotency_key for a retry. On ambiguity, ask the person instead of guessing. An operation that sets a date, target, baseline, owner or self-assessment also needs `basis` saying where that value came from. `assumptions` travels with the diff.", true),
        write("pathbase_propose_plan", "Propose explicit plan operations without inventing dates or applying changes. For a linked conversation integration, pass the linked `conversation_id` and read the graph first; its `plan_version` is required as `expected_base_version`, and read access plus proposal access are both required. The server verifies the workspace link. Without `conversation_id`, expected_base_version is optional; do not call read tools unless pathbase.read is granted. Use existing item IDs for updates and moves; do not match by title alone. Every integration operation needs `match_rationale`. Preserve unmentioned branches, IDs, descendants, execution records and review history; do not delete plan items. An empty integration returns `no_change`. A date, target, baseline, owner or self-assessment also needs `basis` on its operation. Reuse the same idempotency_key for retries; on a version conflict, reread the graph and recompute the proposal.", true),
    ...
    defs.into_iter().map(|shape| {
        ...
        if matches!(shape.name, "pathbase_preview_changes" | "pathbase_propose_plan") {
            props["expected_base_version"]["minLength"] = json!(1);
            props["expected_base_version"]["description"] =
                json!("plan_version from the current authorized graph; stale values are rejected.");
            props["conversation_id"]["description"] = json!(
                "When integrating another conversation, this must be actively linked to this workspace."
            );
            props["operations"]["minItems"] = json!(0);
            props["operations"]["items"]["properties"]["match_rationale"] = json!({
                "type": "string",
                "minLength": 1,
                "maxLength": 500,
        ...
        if let Some(uri) = ui_resource(shape.name) {
            tool["_meta"] = json!({
                ...
                "openai/toolInvocation/invoking": "目標ツリーを読み込み中…",
                "openai/toolInvocation/invoked": "目標ツリーを表示しました",
            });
        }
