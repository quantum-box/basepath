...
Tachyon Cloud AppではWebとRust APIを一つのLambda zipへまとめ、Lambda Web Adapter経由でSPAと同一オリジンの`/api/*`を配信します。現在のSQLiteとセッションはLambdaの一時ストレージ上にあるため、このデプロイはプロトタイプ用途です。コールドスタート・再デプロイでデータやログイン状態が失われる可能性があり、永続ストレージと共有セッションを導入するまでは本番データを保存しないでください。
