...
UI確認用のサンプルデータです。画像に合わせて2025年4月の予定を表示します。Tauriアプリでは目標・取り組み・行動・振り返りをRust API経由でローカル保存し、再起動後も復元します。ブラウザ版は従来どおりページの再読み込みで初期状態に戻ります。アカウント認証、クラウド同期、実通知の送信は未実装です。

## 構成

- `src/App.tsx`: ダッシュボードと操作フロー
- `src/api.ts`: TauriのRust APIとの読み書き
...
- `src-tauri/`: 永続化API、Rustエントリポイント、ウィンドウ設定、アプリアイコン
- `design-qa.md`: 参考画像との比較と検証記録
